...
console.log('メール設定の確認:', {
  ...
  hasAppPassword: !!emailConfig?.apppassword
});
...
// メール送信状態の監視
async function sendMailWithConfirmation(mailOptions: any): Promise<boolean> {
...
export const testSendMail = functions.https.onRequest(async (req, res) => {
  const mailOptions = {
    from: functions.config()['email'].user,
    to: 'yuki.wada@pathoslogos.co.jp', // テスト送信先
    subject: 'テストメール',
...
// テストメール送信用のHTTP関数
export const testEmailSettings = functions.https.onRequest(async (req, res) => {
  console.log('テストメール送信を開始します');
  
  try {
    const mailOptions = {
      from: functions.config().email.user,
      to: functions.config().email.user, // 設定されているメールアドレスに送信
...
// メール設定を確認する関数
export const checkEmailSettings = functions.https.onCall(async (data, context) => {
  ...
  const configStatus = {
    hasUser: !!emailConfig?.user,
    hasAppPassword: !!emailConfig?.apppassword,
    user: emailConfig?.user ? '設定済み' : '未設定',
    appPassword: emailConfig?.apppassword ? '設定済み' : '未設定'
  };
